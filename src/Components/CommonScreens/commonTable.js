...
const StyledTableCell = withStyles(theme => (customStyle.tableCellNetwell))(TableCell);

const StyledTableRow = withStyles(theme => (customStyle.tableRowNetwell))(TableRow);
...
export default  class commonTable extends Component {
    ...
    render() {
        return (
            ...
            <Table  style={{fontFamily : 'Roboto, Arial, Helvetica, sans-serif'}}>
                <TableHead >
                    <TableRow >
                        ...
                        {this.props.RxSimpleShare ?<StyledTableCell></StyledTableCell>:null}
                        {
                            this.props.headerData.map((row,index)=>(
                                index === 0?
                                    <StyledTableCell key={index} style={{width:this.props.sumarryScreenfromEligibility?"auto":'0px'}} >{row}</StyledTableCell>
                                    :
                                    row == "Surcharge"?
                                    // null
                                   ...
                                   :
                                    <StyledTableCell style={{width:this.props.sumarryScreenfromEligibility?"auto":'20px'}} key={index} align="left">{row}</StyledTableCell>
                ...
                <TableBody>
                    ...
                    {this.props.quoteData.map((row,index,key) => (
                            <StyledTableRow key={index} style={( (this.props.totalReq && this.props.ACSM) ? (this.props.quoteData.length - 2 === index || this.props.quoteData.length - 1 === index) : this.props.totalReq ? this.props.quoteData.length - 1 === index : false) ? customStyle.lastRowNetwell : customStyle.otherRow}>
                                                                ...
                                                                <div style={{display:'flex',flexDirection:'column'}}>
                                                                    {
                                                                        this.props.tooltip[index] && this.props.tooltip[index].map((key,j)=>{
                                                                            return(
                                                                                <span style={{ marginTop : '5px'}} key={j+1}>{j+1}. {key}</span>
                                                                            )
                                                                        })
                                                                    }
                                                                </div>
                    ...
                    <StyledTableRow style={{ borderBottom : '2px solid #420045',}}>
                        ...
                        <StyledTableCell colSpan={this.props.headerData.length-2} style={{backgroundColor:'#EAE8DB'}}>(Includes netWell monthly membership dues per household of $11)</StyledTableCell>
